...
根據目前提供的 `generateLearningMaterial()` 程式碼，真正的成本與速度問題有兩層：

1. 沒有先讀 `T` / `U`
   - 目前後端只讀課文 `I`、詞彙 `S`、圖說 `O`、實作 `K`
   - 還沒有把 `T` 欄概念節點與 `U` 欄摘要快取接進流程
...
也就是說，即使只把摘要寫進 `U` 欄，如果其他四項仍然每次都重新生成，整頁仍然會慢，而且 API 成本仍然不低。

## 最新快取方案

本 repo 的最新 patch 已直接實作「全欄位快取」：
