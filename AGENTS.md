...
- `assets/js/aischool-shared.js` 是目前少數正式抽出的共用 runtime，集中管理 session、flash message 與語言 key。
- 多個頁面直接硬編碼不同的 Google Apps Script Web App URL。
- 登入狀態主要靠 `sessionStorage.currentUser` 傳遞，但不同頁面對 session key 與語言 key 的命名並不完全一致。

## Non-Negotiables

- 先確認頁面是否為正式頁面還是 placeholder，再做變更。
- 變更頁面導向時，要同時檢查：
  - 入口頁 `index.html`
...
- 若調整登入或 session 流，至少同步檢查：
  ...
  - `professor.html`
  - 受保護的 professor 子頁
- 若調整 GAS endpoint，不要只改單一頁面；先盤點同功能是否有多個 endpoint 分散在不同頁。
- `student_science_bilingual.html` 目前有 `DEV_BYPASS_LOGIN = true`，除非是刻意做開發模式，不要忽略它。
- 不要把空白 placeholder 頁當成已完成功能。
...
4. 若改到 placeholder 頁升級為正式頁，要明確更新文件中的狀態標記。
